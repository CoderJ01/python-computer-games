...
def inputPlayersLetter():
    ...
    while not (letter == 'X' or letter == '0'):
        ...
        letter = input().upper()
    # The first elemment in the list is the player's letter; the second is the computer's letter.
    if letter == 'X':
        return ['X', 'O']
    else:
        return ['O', 'X']
...
def isSpaceFree(board, move):
    # return True if the passsed move is free on the passed board.
    return board[move] == ' '
...
def chooseRandomMoveFromList(board, movesList):
    ...
    # Returns None if there is no valid response
    possibleMoves = []
    for i in movesList:
...
def getComputerMove(board, computerLetter):
    # Given a board and the computer's timer. determine where to move and return that move.
    if computerLetter == 'X':
        playerLetter = 'O'
    else:
        playerLetter = 'X'
